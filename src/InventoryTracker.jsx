...
const App = () => {
  ...
  useEffect(() => {
    try {
      if (!firebaseConfig || !firebaseConfig.apiKey || firebaseConfig.apiKey === "YOUR_API_KEY") {
        ...
        // We return here to prevent the app from crashing while waiting for config
        return; 
      }
  ...
  const StockForm = ({ onToggleView }) => {
    ...
    return (
      <form onSubmit={handleSubmit} className="p-4 space-y-4 bg-white shadow-xl rounded-xl">
        <h2 className="text-xl font-bold text-gray-700 border-b pb-2 mb-4">Stock Purchase Update</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="lg:col-span-2">
            <label className="block text-sm font-medium text-gray-700">Total Goods Cost (Subtotal)</label>
            <div className="mt-1 p-2 border border-green-300 bg-green-50 rounded-md text-lg font-semibold text-green-700 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 border-t pt-4">
          ...
          <div className="lg:col-span-2">
            <label className="block text-sm font-medium text-gray-700">Overall Total Cost (Final)</label>
            <div className="mt-1 p-2 border border-red-300 bg-red-50 rounded-md text-lg font-semibold text-red-700 shadow-sm">
        ...
        <div className="border-t pt-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">Total Stock Count by Size</label>
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t">
          <button
            ...
            className="flex items-center justify-center rounded-lg bg-indigo-500 px-4 py-2 text-white font-medium hover:bg-indigo-600 transition duration-150 shadow-md"
          >
          ...
          <button
            type="submit"
            disabled={isSaving}
            className="flex items-center justify-center rounded-lg bg-green-600 px-4 py-2 text-white font-medium hover:bg-green-700 transition duration-150 shadow-md disabled:opacity-50"
          >
  ...
  const SaleForm = ({ onToggleView }) => {
    ...
    return (
      <form onSubmit={handleSubmit} className="p-4 space-y-4 bg-white shadow-xl rounded-xl">
        <h2 className="text-xl font-bold text-gray-700 border-b pb-2 mb-4">Sale Transaction Update</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Actual Cost Price (Auto)</label>
            <div className="mt-1 p-2 border border-indigo-300 bg-indigo-50 rounded-md text-lg font-semibold text-indigo-700 shadow-sm">
              ₹ {actualPrice}
            </div>
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t">
          <button
            type="button"
            onClick={onToggleView}
            className="flex items-center justify-center rounded-lg bg-green-500 px-4 py-2 text-white font-medium hover:bg-green-600 transition duration-150 shadow-md"
          >
          ...
          <button
            type="submit"
            disabled={isSaving}
            className="flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2 text-white font-medium hover:bg-indigo-700 transition duration-150 shadow-md disabled:opacity-50"
          >
  ...
  const TrendsView = () => {
    ...
    return (
      <div className="space-y-8">
        <h2 className="text-2xl font-bold text-gray-800">Sales and Inventory Trends</h2>
  ...
  const DataTables = () => {
    ...
    return (
      <div className="space-y-8">
        <h2 className="text-2xl font-bold text-gray-800">Raw Data Records</h2>
        ...
        <Card title="Stock Purchase Records">
          <div className="flex justify-end mb-4">
            <button
              ...
              className="flex items-center rounded-lg bg-red-600 px-4 py-2 text-white font-medium hover:bg-red-700 transition disabled:opacity-50 shadow-md"
            >
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {stocks.map(s => (
                    <tr key={s.id} className="hover:bg-gray-50 transition">
                      ...
                      <TableData className="font-medium text-green-700">{s.costPerPiece?.toFixed(2)}</TableData>
                      ...
                      <TableData className="font-semibold text-red-700">{s.overallTotalCost?.toFixed(2) || s.totalPurchaseAmount?.toFixed(2) || '0.00'}</TableData>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {sales.map(s => (
                    <tr key={s.id} className="hover:bg-gray-50 transition">
                      ...
                      <TableData className="font-medium text-green-700">{s.soldFor?.toFixed(2)}</TableData>
  ...
  const Input = ({ label, name, type = 'text', value, onChange, required = false, min, step }) => (
    <div>
      ...
      <input
        ...
        min={min}
        step={step}
        className="mt-1 block w-full rounded-md border-gray-300 border p-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition"
      />
    </div>
  );
  ...
  const Select = ({ label, name, value, onChange, options, required = false }) => (
    <div>
      ...
      <select
        ...
        onChange={onChange}
        required={required}
        className="mt-1 block w-full rounded-md border-gray-300 border p-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white transition"
      >
        {options.map(option => (
  ...
  const TabButton = ({ view, label, icon: Icon }) => (
    <button
      ...
      className={`px-4 py-2 flex items-center rounded-lg font-medium transition duration-150 ${
        currentView === view
          ? 'bg-white text-indigo-600 shadow-md ring-2 ring-indigo-500'
  ...
  const TableData = ({ children, className = '' }) => (
    <td className={`whitespace-nowrap px-3 py-3 text-sm text-gray-600 ${className}`}>
      {children}
    </td>
  );
